...
import { containers, advancedFields, basicFields, customFields } from './widget-panel/widgets-config'
...
export function createDesigner() {
  ...
  return {
    ...
    appendTableRow(widget) {
      const rowIdx = widget.rows.length// 确定插入行位置
    ...
    appendTableCol(widget) {
      const colIdx = widget.rows[0].cols.length // 确定插入列位置
    ...
    moveUpWidget(parentList, indexOfParentList) {
      console.log(parentList, indexOfParentList)
      if (parentList) {
        if (indexOfParentList === 0) {
          ElMessage(designerText.hint.moveUpFirstChildHint)
          return
        }
    ...
    moveDownWidget(parentList, indexOfParentList) {
      if (parentList) {
        if (indexOfParentList === parentList.length - 1) {
          ElMessage(designerText.hint.moveDownLastChildHint)
          return
        }
